...
class DMX_TempData(PropertyGroup):
    ...
    aditive_selection: BoolProperty(name=_("Add to selection"), description="When selecting a group, add to existing selection", update=onToggleAddSelection, default=True)

    subtractive_selection: BoolProperty(name=_("Remove from selection"), description="When selecting a group, remove from existing selection", update=onToggleSubSelection, default=False)

    keyframe_only_selected: BoolProperty(name=_("Keyframe only selected fixtures (not for autokeying)"), description="Add keyframes with changes only for selected fixtures", default=False)

    mvr_xchange: PointerProperty(name=_("MVR-xchange"), type=DMX_MVR_Xchange)

    release_version_status: StringProperty(name=_("Status"), description="Information about latest release of BlenderDMX", default="Not checked")

    def onUpdateLoggingFilter(self, context):
        DMX_Log.update_filters()

    logging_filter_mvr_xchange: BoolProperty(name=_("MVR-xchange"), default=False, update=onUpdateLoggingFilter)

    logging_filter_dmx_in: BoolProperty(name=_("DMX Data"), default=False, update=onUpdateLoggingFilter)

    logging_filter_fixture: BoolProperty(name=_("Fixture"), default=False, update=onUpdateLoggingFilter)
    ...
    active_subfixtures: CollectionProperty(name=_("Active Subfixtures"), type=PropertyGroup)
